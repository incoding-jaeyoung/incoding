...
function Particles(element, options) {
  ...
  this.options = extend({color:'#fff'}, this.defaults, options);
  this.init();
}
...
const ExploreBtn = ({ onTransitionComplete }) => {
  ...
  useEffect(() => {
    if (buttonRef.current) {
      ...
      particlesRef.current = new Particles(bttn, particlesOpts);
    }
  }, [buttonVisible, onTransitionComplete]);

  const handleClick = () => {
    if (particlesRef.current && !particlesRef.current.isAnimating() && buttonVisible) {
      particlesRef.current.disintegrate();
